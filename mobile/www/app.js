...
;(() => {
  ...
  function renderDateHeader() {
    ...
    const day = d.getDate()
    // 手机顶栏一行要塞下 6 个控件，日期必须写紧凑：同年省年份、跨年用两位年、周几用简称。
    // 之前用「2026 年 9 月 16 日」只剩 ~78px，被截成「202…」。
    const sameYear = y === new Date().getFullYear()
    $('dateTitle').textContent = sameYear ? `${m}月${day}日` : `${String(y).slice(2)}年${m}月${day}日`
    const rel = diffDays(state.today, state.date)
    const relText = rel === 0 ? '今天' : rel === 1 ? '明天' : rel === -1 ? '昨天' : rel > 0 ? `${rel} 天后` : `${-rel} 天前`
    $('dateSub').textContent = `${WEEKDAYS_SHORT[d.getDay()]} · ${relText}`
  ...
  function bindEvents() {
    ...
    // 主题：手机顶栏不再放这个按钮（挪进设置里，给日期腾地方），所以做存在性判断
    if ($('themeBtn')) {
